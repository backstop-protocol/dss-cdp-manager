...
contract PoolTest is BCdpManagerTestBase {
    ...
    function testSimpleBite() public {
        ...
        pool.setProfitParams(1, 100); // 1% goes to member
        // for 10 ether we expect 10/130 * 1.1 = 11/130, from which 1% goes to member
        uint expectedEth = uint(1) * 11 ether / (130 * 100);
        uint expectedEthInJar = uint(99) * 11 ether / (130 * 100);
    ...
    function testFullBite() public {
        ...
        //uint ethBefore = vat.gem("ETH", address(members[0]));
        this.file(address(cat), "ETH", "chop", WAD + WAD/10);
        pool.setProfitParams(2, 100); // 2% goes to member
        // for 26 ether we expect 26/130 * 1.1 = 28.6/130, from which 2% goes to member
        uint expectedEth = uint(2) * 286 ether / (130 * 100 * 10);
        uint expectedEthInJar = uint(98) * 286 ether / (130 * 100 * 10);
        for(uint i = 0 ; i < 4 ; i++) {
            assertTrue(! canKeepersBite(cdp));
        ...
        // jar should get 98% from expectedEthInJar * 4
    ...
    function testBiteInPartsThenUntopNonOneRate() public {
        ...
        this.file(address(cat), "ETH", "chop", WAD + WAD/10);
        pool.setProfitParams(65, 1000); // 6.5% goes to member
    ...
    function testFullBiteWithRate() public {
        ...
        // for 26 ether we expect 26/140 * rate * 1.1 = 28.6/140 * rate, from which 2% goes to member
        uint expectedEth = uint(2) * 286 ether * currRate / (100 * 1400 * RAY);
    ...
    function testFullBiteWithRateAndDust() public {
        ...
        // for 26 ether we expect 26/140 * rate * 1.1 = 28.6/140 * rate, from which 98% goes to jar
        jug.drip("ETH");
        (, uint currRate,,,) = vat.ilks("ETH");
        uint expectedEth = uint(2) * amt * 11 * currRate / (100 * 1400 * RAY);
        uint expectedEthInJar = uint(98) * amt * 11 * currRate / (100 * 1400 * RAY);
        ...
        // jar should get 98% from 104 * 1.1 * 1.1 / 140
        assertTrue(vat.gem("ETH", address(jar)) >= expectedEthInJar * 4);
    }
    ...
    function testHighDaiRateInUsd() public {    
        ...
        //uint ethBefore = vat.gem("ETH", address(members[0]));
        this.file(address(cat), "ETH", "chop", WAD + WAD/10);
        pool.setProfitParams(7, 100); // 7% goes to member
        
        // for 26 ether we expect 26/130 * 1.1 = 28.6 / 130, from which 7% goes to member
        uint _100Percent = 286 ether / (130 * 10);
        // _100Percent * 7% * 1.03
        uint expectedEth = _100Percent * 7 * 103 / (100 * 100);
        ...
        for(uint i = 0 ; i < 4 ; i++) {
            ...
            assertEq(uint(dink), expectedEth);
            assertEq(vat.gem("ETH", address(members[i])), expectedEth);
            assertEq(vat.gem("ETH", address(jar)), expectedEthInJar * (i+1));
            (uint cdpArt, uint cdpCushion, address[] memory winners, uint[] memory bite) = pool.getCdpData(cdp);
            cdpArt; //shh
            cdpCushion; //shh
            winners; //shh
        ...
        // jar should get 93% from expectedEthInJar * 4
        assertEq(vat.gem("ETH", address(jar)), expectedEthInJar * 4);
    }
    ...
    function testLowDaiRateInUsd() public {
        ...
        uint _100Percent = 286 ether / (130 * 10);
        // _100Percent * 7% * 0.97
        uint expectedEth = _100Percent * 7 * 97 / (100 * 100);
        uint expectedEthInJar = _100Percent - expectedEth;
    ...
    // 1 DAI = 1.15 USD
    function testUnexpectedHighRateInUsd() public {    
        members[0].doDeposit(pool, 1000 ether * RAY);
        members[1].doDeposit(pool, 950 ether * RAY);
        members[2].doDeposit(pool, 900 ether * RAY);
        members[3].doDeposit(pool, 850 ether * RAY);
        ...
        uint _100Percent = 286 ether / (130 * 10);
        // _100Percent * 7% * 1.15
        uint expectedEth = _100Percent * 7 * 115 / (100 * 100);
        uint expectedEthInJar = 0;
        //assertEq(expectedEthInJar, 0);
        ...
        // jar should get 93% from expectedEthInJar = 0
        assertEq(vat.gem("ETH", address(jar)), expectedEthInJar);
    }

    // 1 DAI = 0.60 USD
    function testUnexpectedLowRateInUsd() public {    
        members[0].doDeposit(pool, 1000 ether * RAY);
        members[1].doDeposit(pool, 950 ether * RAY);
        members[2].doDeposit(pool, 900 ether * RAY);
        members[3].doDeposit(pool, 850 ether * RAY);
        ...
        // Update DAI to USD rate
        FakeDaiToUsdPriceFeed dai2usd = FakeDaiToUsdPriceFeed(address(pool.dai2usd()));
        dai2usd.setPrice(0.6 * 1e18); // 1 DAI = 0.6 USD
        ...
        uint _100Percent = 286 ether / (130 * 10);
        // _100Percent * 7% * 0.6
        uint expectedEth = _100Percent * 7 * 6 / (100 * 10);
        uint expectedEthInJar = _100Percent - expectedEth;
